 ...
 * VirtualMachines are handled by the VmAllocationPolicySteady.
 * 
 * @author Rodrigo N. Calheiros
 * @author Anton Beloglazov
 * @since CloudSim Toolkit 1.0
 */
public class DatacenterSteady extends SimEntity {

	/** The characteristics. */
	private DatacenterCharacteristicsSteady characteristics;

	/** The regional cis name. */
	private String regionalCisName;

	/** The vm provisioner. */
	private VmAllocationPolicySteady vmAllocationPolicy;
	...
	/** The vm list. */
	private List<? extends VmSteady> vmList;
	...
	public DatacenterSteady(
		...
		for (HostSteady host : getCharacteristics().getHostList()) {
	...
	protected void processVmCreate(SimEvent ev, boolean ack) {
		VmSteady vm = (VmSteady) ev.getData();
	...
	protected void processVmMigrate(SimEvent ev, boolean ack) {
		...
		HostSteady host = (HostSteady) migrate.get("host");
		...
		Log.formatLine(
				"%.2f: Migration of VM #%d to HostSteady #%d is completed",
				CloudSim.clock(),
				vm.getId(),
				host.getId());
	...
	protected void processCloudletMove(int[] receivedData, int type) {
		...
		if (cl == null) {// cloudlet doesn't exist
			...
			if (destId == getId()) {
				VmSteady vm = getVmAllocationPolicy().getHost(vmDestId, userId).getVm(vmDestId,userId);
				if (vm == null) {
					failed = true;
				} else {
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			...
			HostSteady host = getVmAllocationPolicy().getHost(vmId, userId);
			VmSteady vm = host.getVm(vmId, userId);
	...
	protected void updateCloudletProcessing() {
		...
		if (CloudSim.clock() < 0.111 || CloudSim.clock() > getLastProcessTime() + CloudSim.getMinTimeBetweenEvents()) {
			List<? extends HostSteady> list = getVmAllocationPolicy().getHostList();
			double smallerTime = Double.MAX_VALUE;
			// for each host...
			for (int i = 0; i < list.size(); i++) {
				HostSteady host = list.get(i);
	 ...
	 */
	protected DatacenterCharacteristicsSteady getCharacteristics() {
	 ...
	 */
	protected void setCharacteristics(DatacenterCharacteristicsSteady characteristics) {
	 ...
	 */
	public VmAllocationPolicySteady getVmAllocationPolicy() {
	 ...
	 */
	protected void setVmAllocationPolicy(VmAllocationPolicySteady vmAllocationPolicy) {
