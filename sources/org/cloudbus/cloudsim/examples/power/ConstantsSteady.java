...
//		}
	...
	private static int[] getVmMips(int[][] vmCtSla) {
		...
		int bufferSec = 0;//extra 60 seconds
		int maxMips = (int) Math.ceil(2256);//i7, E5507
...
//			vmMipsInt[i][1]=i%CLOUDLET_LENGTH_PRIVATE.length;
			int calculatedMips = (int) Math.ceil(CLOUDLET_LENGTH_PRIVATE[i%CLOUDLET_LENGTH_PRIVATE.length]*1000/(vmCtSla[i%CLOUDLET_LENGTH_PRIVATE.length][i/CLOUDLET_LENGTH_PRIVATE.length]-bufferSec));
//			if((vmCtSla[i%CLOUDLET_LENGTH_PRIVATE.length][i/CLOUDLET_LENGTH_PRIVATE.length] <= 6*60*60)){ //6 hours
	//			if(calculatedMips > maxMips){
					//---*1000-CEA			
					vmVmSlaCloudletListSteady.add(new VmSlaCloudletListSteady(i,calculatedMips,vmCtSla[i%CLOUDLET_LENGTH_PRIVATE.length][i/CLOUDLET_LENGTH_PRIVATE.length],CLOUDLET_LENGTH_PRIVATE[i%CLOUDLET_LENGTH_PRIVATE.length]*1000));
//				} else {
					//Max Mips			
//					vmVmSlaCloudletListSteady.add(new VmSlaCloudletListSteady(i,maxMips,vmCtSla[i%CLOUDLET_LENGTH_PRIVATE.length][i/CLOUDLET_LENGTH_PRIVATE.length],CLOUDLET_LENGTH_PRIVATE[i%CLOUDLET_LENGTH_PRIVATE.length]*1000)); 
//				}
//			} else {
//				if(calculatedMips > 3400/10 && calculatedMips * 1.1 <= 3400){ //i7, E5507
//					//---*1000-CEA			
//					vmVmSlaCloudletListSteady.add(new VmSlaCloudletListSteady(i,(int)(calculatedMips*1.1),vmCtSla[i%CLOUDLET_LENGTH_PRIVATE.length][i/CLOUDLET_LENGTH_PRIVATE.length],CLOUDLET_LENGTH_PRIVATE[i%CLOUDLET_LENGTH_PRIVATE.length]*1000));
//				} else {
//					//Max Mips			
//					vmVmSlaCloudletListSteady.add(new VmSlaCloudletListSteady(i,maxMips,vmCtSla[i%CLOUDLET_LENGTH_PRIVATE.length][i/CLOUDLET_LENGTH_PRIVATE.length],CLOUDLET_LENGTH_PRIVATE[i%CLOUDLET_LENGTH_PRIVATE.length]*1000)); //E5507, i7
//				}
//			}
...
//		VmSlaCloudletListListSteady.copy(vmVmSlaCloudletListSteady);
		...
		if(VM_MIPS_SORTING==true){
			...
			if(VM_SLA_SORTING_ASCENDING==true){//System.out.println("descending mips");
				VmSlaCloudletListSteady.sortByExpectedTime(vmVmSlaCloudletListSteady);
				//Collections.sort(vmMips,Collections.reverseOrder());
				}
			else{//System.out.println("ascending mips");
				VmSlaCloudletListSteady.sortByExpectedTimeDescending(vmVmSlaCloudletListSteady);
				//Collections.sort(vmMips);
				}
		}
		VmSlaCloudletListListSteady.copy(vmVmSlaCloudletListSteady);
		...
		for (VmSlaCloudletListSteady tuple : vmVmSlaCloudletListSteady){
			...
			String xx = (tuple.getExpectedTime()>tuple.getSla())?"***"+(tuple.getExpectedTime()-tuple.getSla()):" ";
			System.out.println("#"+(index-1) + "\t" + (vmMipsInt[index-1])+"\t"+(int)(tuple.getCloudletId()/tuple.getSla()) +"\t"+tuple.getSla() + "\t" + tuple.getExpectedTime() + "\t" + xx );
		}
//		for (int i=0; i<vmCtSla[0].length*CLOUDLET_LENGTH_PRIVATE.length;i++){
